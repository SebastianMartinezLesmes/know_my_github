...
export class GitHubPage implements OnInit {
  ...
  getReposLanguages() {
    this.repos.forEach((repo: any) => {
      this.http.get<Languages>('https://api.github.com/repos/' + this.userName + '/' + repo.name + '/languages', { headers: this.getHeaders() })
        .subscribe(
          (data: Languages) => {
            const totalLines = Object.values(data).reduce((sum, value) => sum + value, 0);
            repo.languages = Object.keys(data).map((lang) => ({
              name: lang,
              percentage: ((data[lang] / totalLines) * 100).toFixed(2) // Calcula el porcentaje
            }));
